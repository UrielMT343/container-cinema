...
const holdMutation = useMutation<HeldTicket[], Error, void>({
  ...
  onError: (err: unknown) => {
    ...
    if (status === 409) {
      holdError.value = message ?? 'One or more selected seats were just taken. Please try again.';
    } else if (status === 400) {
      holdError.value = message ?? 'Invalid selection. Please check your seats and try again.';
    } else {
      holdError.value = message ?? 'Failed to hold seats. Please try again.';
    }
  },
});
...
<template>
  <div>
    ...
    <Transition name="slide-fade">
      ...
      >
        You can select a maximum of {{ MAX_SELECTION }} seats. Deselect a seat before choosing another.
      </div>
    </Transition>
    ...
    <div class="overflow-x-auto pb-4">
      <div class="min-w-[800px] grid grid-cols-[auto_repeat(15,minmax(0,1fr))] gap-2 text-center mx-auto">
        ...
        <template v-for="rowData of rowSeats" :key="rowData.row">
          ...
          <button
            ...
            :key="index"
            :disabled="seat?.status === 'SOLD' || seat?.status === 'HOLD'"
            :class="{
              'bg-green-100 text-green-700 border-green-200 hover:bg-green-200': seat?.status === 'AVAILABLE' && !isSelected(seat),
              'bg-blue-600 text-white border-blue-700 shadow-md': isSelected(seat),
              'bg-yellow-100 text-yellow-700 border-yellow-200': seat?.status === 'HOLD',
              'bg-red-100 text-red-700 border-red-200 opacity-60 cursor-not-allowed': seat?.status === 'SOLD',
              'bg-transparent border-transparent text-gray-200 cursor-default': !seat,
      ...
      <div class="mt-8 flex gap-6 justify-center flex-wrap">
        ...
        <div class="flex items-center gap-2">
          <div class="w-4 h-4 rounded border border-blue-700 bg-blue-600"></div>
    ...
    <div class="mt-8 flex flex-col items-center gap-3">
      <button
        :disabled="selectedSeats.length === 0 || holdMutation.isPending.value"
      ...
      >
        <span v-if="holdMutation.isPending.value">Holding seats...</span>
